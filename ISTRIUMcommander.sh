...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#                                                     #
#                 Istrium Commander Script            #
#                      by Philconee                   #
#     thanks ViperTKD & tharude for the ARKcommander  #
#                 28/01/2018 Istrium Team             #
#                                                     #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
if [ "$(id -u)" = "0" ]; then
    ...
    echo -e "\nUse $(green "bash ISTRIUMcommander.sh") as a REGULAR user instead"
    echo -e "Execute ONCE $(green "chmod +x ISTRIUMcommander.sh") followed by $(green "ENTER")"
    echo -e "and start it only by $(green "./ISTRIUMcommander.sh") as regular user after\n"
    exit 1
fi
...
if [ $(systemd-detect-virt -c) != "none" ]; then
    ...
    echo "$(ired "                                                                                 ")"
    echo "$(ired "     Running Istrium Node on a Container based virtual system is not recommended!")"
...
d888888b .d8888. d888888b d8888b. d888888b db    db .88b  d88. 
  `88'   88'  YP `~~88~~' 88  `8D   `88'   88    88 88'YbdP`88 
   88    `8bo.      88    88oobY'    88    88    88 88  88  88 
   88      `Y8b.    88    88`8b      88    88    88 88  88  88 
  .88.   db   8D    88    88 `88.   .88.   88b  d88 88  88  88 
Y888888P `8888Y'    YP    88   YD Y888888P ~Y8888P' YP  YP  YP 
...
function net_height {
    local heights=$(curl -s "http://164.132.197.122:4100/api/peers" | jq -r '.peers[] | .height')
...
function proc_vars {
        node=`pgrep -a "node" | grep genesis | awk '{print $1}'`
        if [ "$node" == "" ] ; then
                node=0
        fi
        ...
        arkdir=`locate -b "\Istrium"`

        # Getting the parent of the install path
        parent=`dirname $arkdir 2>&1`

        # Forever Process ID
        forever_process=`forever --plain list | grep genesis | sed -nr 's/.*\[(.*)\].*/\1/p'`
...
#PSQL Queries
query() {
PUBKEY="$(psql -d istrium_mainnet -t -c 'SELECT ENCODE("publicKey",'"'"'hex'"'"') as "publicKey" FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
DNAME="$(psql -d istrium_mainnet -t -c 'SELECT username FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
PROD_BLOCKS="$(psql -d istrium_mainnet -t -c 'SELECT producedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
MISS_BLOCKS="$(psql -d istrium_mainnet -t -c 'SELECT missedblocks FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
#BALANCE="$(psql -d istrium_mainnet -t -c 'SELECT (balance/100000000.0) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | sed -e 's/^[[:space:]]*//')"
BALANCE="$(psql -d istrium_mainnet -t -c 'SELECT to_char(("balance"/100000000.0), '"'FM 999,999,999,990D00000000'"' ) as balance FROM mem_accounts WHERE "address" = '"'"$ADDRESS"'"' ;' | xargs)"
HEIGHT="$(psql -d istrium_mainnet -t -c 'SELECT height FROM blocks ORDER BY HEIGHT DESC LIMIT 1;' | xargs)"
RANK="$(psql -d istrium_mainnet -t -c 'WITH RANK AS (SELECT DISTINCT "publicKey", "vote", "round", row_number() over (order by "vote" desc nulls last) as "rownum" FROM mem_delegates where "round" = (select max("round") from mem_delegates) ORDER BY "vote" DESC) SELECT "rownum" FROM RANK WHERE "publicKey" = '"'03cfafb2ca8cf7ce70f848456b1950dc7901946f93908e4533aace997c242ced8a'"';' | xargs)"
}
...
#   echo -e "$(green "Public Key:")\n$(yellow "$PUBKEY")\n"
    ...
    echo -e "$(green "      ISTRIUM Balance  : ")$(yellow "$BALANCE")"
    echo
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
        if [ -e $arkdir/app.js ]; then
                echo -e "\n$(green "       ✔ ISTRIUM Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      ISTRIUM Node process is running with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $arkdir")\n"
                else
                        echo -e "\n$(red "       ✘ No ISTRIUM Node process is running")\n"
                fi
        else
                echo -e "\n$(red "       ✘ No ISTRIUM Node installation is found")\n"
        fi
    echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
...
function ntpd {
        ...
        if [ $(systemd-detect-virt) == "lxc" ] || [ $(systemd-detect-virt) == "openvz" ]; then
            ...
            sleep 2
                                if ! sudo pgrep -x "ntpd" > /dev/null; then
                                        echo -e "NTP failed to start! It should be installed and running for ISTRIUM.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
                                        exit 1
                                fi
...
function log_rotate {
    if [[ "$(uname)" == "Linux" ]]; then
        ...
        if [ ! -f /etc/logrotate.d/ark-logrotate ]; then
            echo -e " Setting up Logrotate for ISTRIUM node log files."
...
function git_upd_check {
    ...
    if [ -d "$arkdir" ]; then
        ...
        if [ "$LOCAL" == "$REMOTE" ]; then
            echo -e "         $(igreen "    ISTRIUM Node is Up-to-date    \n")"
            UP_TO_DATE=1
...
function purge_pgdb {
    ...
    echo -e "    $(ired "   running ISTRIUM Node processes and will  ")"
...
if [ "$(ls -A $SNAPDIR)" ]; then
    if [[ $(expr `date +%s` - `stat -c %Y $SNAPDIR/current`) -gt 900 ]]; then
        echo -e "$(yellow " Existing Current snapshot is older than 15 minutes")"
            read -e -r -p "$(yellow "\n Download from ISTRIUM.IO? (Y) or use Local (N) ")" -i "Y" YN
            if [[ "$YN" =~ [Yy]$ ]]; then
                echo -e "$(yellow "\n     Downloading latest snapshot from ISTRIUM.IO\n")"
                rm $SNAPDIR/current
        ...
        ## Numeric checks
            ...
            pg_restore -O -j 8 -d istrium_mainnet $SNAPDIR/${snapshots[$(($REPLY-1))]} 2>/dev/null
        ...
        if [[ "$YN" =~ [Yy]$ ]]; then
        echo -e "$(yellow "\n     Downloading current snapshot from ISTRIUM.IO\n")"
                wget -nv https://snapshots.ark.io/current -O $SNAPDIR/current
        echo -e "$(yellow "\n              Download finished\n")"
        fi
        ...
        if [[ $? -eq 0 ]]; then
                ...
                echo -e "$(yellow "\n   Restoring $SNAPDIR/current ... ")"
                                pg_restore -O -j 8 -d istrium_mainnet $SNAPDIR/current 2>/dev/null
...
#   proc_vars
    cd $HOME
    git clone https://github.com/Philconee/Istrium.git 2>/dev/null
    cd Istrium
...
function create_db {
        ...
        sleep 1
#       sudo -u postgres dropdb --if-exists istrium_mainnet
#       sleep 1
...
function db_exists {
        ...
        if [[ ! $(sudo -u postgres psql istrium_mainnet -c '\q' 2>&1) ]]; then
...
function drop_db {
        ...
        dropdb --if-exists istrium_mainnet
}
...
#           forever start app.js --genesis genesisBlock.mainnet.json --config config.mainnet.json
        ...
        echo "ISTRIUM Node is already up to date!"
        sleep 2
    fi
}
...
function inst_w {
    asciiart
    echo -e "\n$(green " Downloading from git ...")\n"
    git clone https://github.com/Philconee/Istrium-desktop.git
    echo -e "\n$(green " Installing ISTRIUM Wallet ...")\n"
    cd Istrium-desktop
    npm install
    pause
}
...
install_wallet(){
    ...
    clear
    asciiart
    echo -e "$(green "          ✔ ISTRIUM wallet was installed")\n"
}
...
one(){
    ...
    if [ -e $arkdir/app.js ]; then
        clear
        asciiart
        echo -e "\n$(green "       ✔ ISTRIUM Node is already installed!")\n"
        if [ "$node" != "" ] && [ "$node" != "0" ]; then
                    echo -e "$(green "A working instance of ISTRIUM Node is found with:")"
                    echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                    echo -e "$(green "and Work Directory: $arkdir")\n"
                fi
        pause
        ...
        clear
        asciiart
        echo -e "$(yellow "           Installing ISTRIUM node....")"
        create_db
        inst_ark
        clear
        asciiart
        echo -e "$(green "          ✔ ISTRIUM node was installed")\n"
        sudo updatedb
        sleep 1
        proc_vars
        log_rotate
...
two(){
    clear
    asciiart
    echo -e "$(ired "!!! This option will erase your DB and ISTRIUM Node installation !!!")\n"
    ...
    if [ "$keys" == "Y" ]; then
        proc_vars
            if [ -e $arkdir/app.js ]; then
                    clear
                    asciiart
                    echo -e "\n$(green " ✔ ISTRIUM Node installation found in $arkdir")\n"
                    if [ "$node" != "" ] && [ "$node" != "0" ]; then
                            echo -e "$(green "A working instance of ISTRIUM Node is found with:")"
                            echo -e "$(green "System PID: $node, Forever PID $forever_process")"
                echo -e "$(yellow "           Stopping ISTRIUM node ...")\n"
                cd $arkdir
            ...
            echo -e "$(yellow "        Removing ISTRIUM Node directory...")\n"
            sleep 1
            rm -rf $arkdir
            drop_db
            drop_user
...
#               echo "Break1"; pause
                if [ "$keys" == "Y" ]; then
                    ...
                    read -e -r -p "$(yellow " Do you want to start ISTRIUM Node now? (Y/N): ")" -i "Y" keys
                    if [ "$keys" == "Y" ]; then
                        start
                    fi
                else
...
three(){
    ...
    if [ "$UP_TO_DATE" -ne 1 ]; then
            ...
            update_ark
                    ...
                    echo -e "\n$(green "    ✔ ISTRIUM Node was successfully restarted")\n"
                    pause
        else
                    echo -e "\n$(red "       ✘ ISTRIUM Node process is not running")\n"
            echo -e "$(green "            Updating ISTRIUM Node...")\n"
            update_ark
            forever start app.js --genesis genesisBlock.mainnet.json --config config.mainnet.json >&- 2>&-
            echo -e "$(green "    ✔ ISTRIUM Node was successfully started")\n"
                    pause
            fi
    else
            echo -e "         $(igreen " ISTRIUM Node is already Up-to-date \n")"
            sleep 2
    fi
...
four(){
        ...
        snap_menu
                echo -e "$(green "            Starting ISTRIUM Node...")"
        forever start app.js --genesis genesisBlock.mainnet.json --config config.mainnet.json >&- 2>&-
                echo -e "\n$(green "    ✔ ISTRIUM Node was successfully started")\n"
                pause
        else
                echo -e "\n$(red "       ✘ ISTRIUM Node process is not running")\n"
...
five(){
    ...
    if [ "$keys" == "Y" ]; then
            if [ "$node" != "" ] && [ "$node" != "0" ]; then
            echo -e "\n$(green "       Instance of ISTRIUM Node found with:")"
            echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "       Directory: $arkdir")\n"
            echo -e "$(green "                Restarting...")"
                    forever restart $forever_process >&- 2>&-
...
start_w(){
    cd
    if [ -e Istrium-desktop/main.js ]; then
	clear
	asciiart
	echo -e "\n$(green "       ✔ ISTRIUM Wallet installation found!")\n"
	cd Istrium-desktop
	npm start
    else
	echo -e "\n$(red "       ✘ No ISTRIUM Wallet installation found!")\n"
    fi
    pause
}
...
start(){
    cd
        proc_vars
        if [ -e Istrium/app.js ]; then
                clear
                asciiart
                echo -e "\n$(green "       ✔ ISTRIUM Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green " A working instance of ISTRIUM Node was found with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $arkdir")\n"
        else
            echo -e "$(green "            Starting ISTRIUM Node...")\n"
            ...
            sleep 1
            proc_vars
            echo -e "\n$(green "       ISTRIUM Node started with:")"
            echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
            echo -e "$(green "   and Work Directory: $arkdir")\n"
                fi
    else
...
status(){
        ...
        if [ -e $arkdir/app.js ]; then
                ...
                echo -e "\n$(green "       ✔ ISTRIUM Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      ISTRIUM Node process is working with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $arkdir")\n"
                else
                        echo -e "\n$(red "       ✘ No ISTRIUM Node process is running")\n"
    ...
    cd
        ...
        if [ -e Istrium/app.js ]; then
                ...
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        ...
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $arkdir")\n"
            echo -e "$(green "            Stopping ISTRIUM Node...")\n"
            cd $arkdir
            forever stop $forever_process >&- 2>&-
            cd $parent
            echo -e "$(green "    ✔ ISTRIUM Node was successfully stopped")\n"
                else
            echo -e "\n$(red "       ✘ No ISTRIUM Node process is running")\n"
                fi
        else
...
log(){
    ...
    proc_vars
    trap : INT
    tail -f Istrium/logs/istrium.log
    pause
}
...
show_menus() {
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "              1. Install ISTRIUM"
    echo "              2. Reinstall ISTRIUM"
    echo "              3. Update ISTRIUM"
    echo "              4. Rebuild Database"
    echo "              5. Set/Reset Secret"
    echo "              6. OS Update"
    echo "              7. Install ISTRIUM Wallet"
    echo
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "              A. ISTRIUM Start"
    echo "              B. ISTRIUM Wallet Start"
    echo "              R. Restart ISTRIUM"
    echo "              K. Kill ISTRIUM"
    echo "              S. Node Status"
        echo "              L. Node Log"
    echo "              0. Exit"
...
sub_menu() {
    ...
    echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
    echo "           1. Install ISTRIUM Cli"
    echo "           2. Install ISTRIUM Explorer"
...
read_options(){
    local choice
    read -p "        Enter choice [1 - 7,A,B,R,K,S,L]: " choice
    case $choice in
        1) one ;;
        2) two ;;
        3) three ;;
